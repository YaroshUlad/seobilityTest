...
export const validation = (values: ValuesType) => {
	...
	if ( !values.fullName) {
		...
		errors.fullName = 'First and last name should includes 3-30 characters with only one space';
	}
	if ( !values.email) {
	...
	} else if ( !/^\+?(\d{1,3})?[- .]?\(?(?:\d{2,3})\)?[- .]?\d\d\d[- .]?\d\d[- .]?\d\d$/.test(values.phone)) {
